...
@property (nonatomic, copy, readonly) JRObjectBlockDefine(T, InDB);///< the database : parameter is FMDatabase: InDB(db)
...
@property (nonatomic, copy, readonly) JRBoolBlockDefine(T, Cache);///< cache condition, NO by default
- (instancetype)Cached;///< equal to Cache(YES)
- (instancetype)NoCached;///< equal to Cache(NO)
...
@property (nonatomic, copy, readonly) JRCompleteBlockDefine(T, Complete);
 ...
 *  @param complete
 */
- (JRDBResult *)exe:(JRDBChainComplete _Nullable)complete;
- (JRDBResult *)exe;
